...
export function InvoiceCreator({ editingInvoice, onBack }: InvoiceCreatorProps) {
  ...
  const [invoiceData, setInvoiceData] = useState<InvoiceData>({
    ...
    discountType: (editingInvoice as any)?.discountType || "percentage",
    discountValue: (editingInvoice as any)?.discountValue ?? 0,
    discountAmount: (editingInvoice as any)?.discountAmount || 0,
    currency: (editingInvoice as any)?.currency || "INR",
    notes: editingInvoice?.notes || "",
    termsAndConditions: editingInvoice?.termsAndConditions || "",
    thankYouNote: (editingInvoice as any)?.thankYouNote || "Thank you for choosing DREAM TEAM SERVICES. We are committed to delivering high-quality services. Please feel free to reach out for any clarifications during the Project.",
    ...
    swiftCode: (editingInvoice as any)?.swiftCode || "BARBINBBKKD",
